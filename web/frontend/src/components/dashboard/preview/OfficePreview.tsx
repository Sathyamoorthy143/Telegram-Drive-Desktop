...
/**
 * Fully client-side Office preview — no external viewer service, works on
 * localhost / self-hosted installs:
...
export function OfficePreview({ file, activeFolderId, variant }: OfficePreviewProps) {
        ...
        const load = async () => {
            try {
                ...
                if (blob.size > MAX_BYTES) {
                    setState({ status: 'error', message: 'File too large to preview in browser (>25MB). Download it instead.' });
                    return;
                }
                ...
                if (variant === 'doc') {
                    if (ext === 'docx') {
                        ...
                        setState({ status: 'error', message: `Legacy .${ext} files can't be previewed in the browser.` });
                    }
                    return;
                }
    ...
    if (state.status === 'error') {
        return <PreviewError message={state.message} detail="Use the Download button above to open it in a desktop app." />;
    }
