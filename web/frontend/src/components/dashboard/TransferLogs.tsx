...
import { X, History, FileUp, FileDown, Search, Trash2, Share2, Star, Tag } from 'lucide-react';
...
export function TransferLogs({ onClose }: TransferLogsProps) {
    ...
    const filteredActivity = activity.filter((a: any) => (a.name || a.detail || a.action || '').toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    const iconFor = (action: string) => {
        if (action.includes('upload')) return <FileUp className="w-4 h-4 text-blue-400" />;
        if (action.includes('download')) return <FileDown className="w-4 h-4 text-telegram-secondary" />;
        if (action.includes('share')) return <Share2 className="w-4 h-4 text-green-400" />;
        if (action.includes('star')) return <Star className="w-4 h-4 text-yellow-400" />;
        if (action.includes('tag')) return <Tag className="w-4 h-4 text-purple-400" />;
        return <History className="w-4 h-4 text-telegram-subtext" />;
    };
...
<div className="flex items-center gap-3">
                         ...
                         <h2 className="text-lg font-bold text-telegram-text">Activity Log</h2>
                     </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-6">
                    {filteredActivity.length > 0 && (
                        <div>
                            <h3 className="text-[10px] uppercase tracking-widest text-telegram-subtext font-bold mb-2">Activity (synced)</h3>
                            <div className="space-y-1">
                                {filteredActivity.slice(0, 50).map((a: any, i: number) => (
                                    <div key={a.id ?? i} className="flex items-center gap-3 px-3 py-2 hover:bg-white/5 rounded-lg">
                                        {iconFor(a.action || '')}
                                        <span className="text-xs font-bold uppercase text-telegram-subtext w-20">{a.action}</span>
                                        <span className="text-sm text-telegram-text truncate flex-1">{a.name || a.detail || ''}</span>
                                        <span className="text-[10px] text-telegram-subtext font-mono">{a.created_at ? new Date(a.created_at).toLocaleString() : ''}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                    )}
                    {filteredLogs.length === 0 && filteredActivity.length === 0 ? (
