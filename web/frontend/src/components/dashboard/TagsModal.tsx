...
export function TagsModal({ file, onClose }: { file: any; onClose: () => void }) {
  ...
  useEffect(() => {
    api.getTags(file.id, file.folder_id ?? undefined).then(setTags).catch(()=>{}).finally(()=>setLoading(false));
  }, [file]);
  const add = () => {
  ...
  const save = async () => {
    try { await api.setTags(file.id, file.folder_id ?? undefined, tags); toast.success('Tags saved'); onClose(); }
