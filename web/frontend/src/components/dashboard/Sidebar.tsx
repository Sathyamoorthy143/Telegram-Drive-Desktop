                ...
                <div key={node.id}>
                    <div className="group flex items-center pr-2">
                        {node.children.length > 0 ? (
                            <button onClick={() => toggle(node.id)} className="p-1 hover:bg-white/5 rounded text-telegram-subtext transition-colors">
        ...
        <aside className="w-64 bg-telegram-surface border-r border-telegram-border flex flex-col overflow-x-hidden" onClick={e => e.stopPropagation()}>
            ...
            <nav className="flex-1 px-2 py-4 space-y-1 overflow-y-auto min-h-0">
                ...
                <SidebarItem
                    icon={Star}
                    label="Starred"
                    active={activeFolderId === -2}
                    onClick={() => setActiveFolderId(-2)}
                ...
                <SidebarItem
                    icon={Clock}
                    label="Recent"
                    active={activeFolderId === -3}
                    onClick={() => setActiveFolderId(-3)}
                    onDrop={(e: React.DragEvent) => e.preventDefault()}
                    onContextMenu={(e) => e.preventDefault()}
                    folderId={-3}
                />
