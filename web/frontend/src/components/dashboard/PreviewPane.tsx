...
export function PreviewPane({ file, onClose, onDownload, onDelete, onProperties, onOpenFolder }: PreviewPaneProps) {
    if (!file) {
        return (
        ...
        <div className="w-80 border-l border-telegram-border bg-telegram-surface flex flex-col animate-in slide-in-from-right duration-300">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            {isFolder ? (
                                <button
                                    ...
                                    className="flex flex-col items-center gap-2 p-3 bg-telegram-primary/10 hover:bg-telegram-primary/20 text-telegram-primary rounded-xl transition-all border border-telegram-primary/20 col-span-2"
                                >
                                    <FolderOpen className="w-5 h-5" />
                                    <span className="text-[10px] font-bold uppercase">Open Folder</span>
                                </button>
                            ) : (
                                <button
                            ...
                            <button
                                ...
                                className={`flex flex-col items-center gap-2 p-3 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-xl transition-all border border-red-500/20 ${isFolder ? 'col-span-2' : ''}`}
                            >
            ...
            <div className="p-4 bg-telegram-hover/20 border-t border-telegram-border">
                <button
                    ...
                    className="w-full py-2.5 bg-telegram-surface border border-telegram-border hover:bg-telegram-hover text-telegram-text rounded-lg text-xs font-bold uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-sm"
                >
