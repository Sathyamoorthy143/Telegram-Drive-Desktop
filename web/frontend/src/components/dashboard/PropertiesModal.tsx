...
import { X, Calendar, Hash, HardDrive, Folder, Info, Loader2, Tag } from 'lucide-react';
...
export function PropertiesModal({ file, onClose }: PropertiesModalProps) {
    ...
    useEffect(() => {
        if (file.type === 'folder') {
            setLoading(true);
            api.getFolderProperties(file.id)
                .then(setExtra)
                        ...
                        {file.type !== 'folder' && (
                           <div className="pt-2 space-y-2">
                                <div className="flex items-center gap-1 flex-wrap">
                                    ...
                                    {tags.length === 0 ? <span className="text-[10px] text-telegram-subtext">No tags</span> :
                                      tags.map(t => <span key={t} className="px-2 py-0.5 bg-purple-500/15 text-purple-300 rounded text-[10px]">#{t}</span>)}
                                </div>
